...
export default function EditarNoticiaPage() {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      if (dbErr || !data) {
        console.error('[Noticias] db.select:', dbErr)
        ...
        setForm({
          ...
          fecha_publicacion: data.fecha_publicacion
            ? data.fecha_publicacion.slice(0, 10)
            : '',
  ...
  const handleSubmit = async (e: React.BaseSyntheticEvent) => {
    ...
    if (dbErr) {
      console.error('[Noticias] db.update:', dbErr)
    ...
    <div className="max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          {form.titulo && <p className="text-gray-400 text-sm truncate max-w-md">{form.titulo}</p>}
        </div>
      </div>
