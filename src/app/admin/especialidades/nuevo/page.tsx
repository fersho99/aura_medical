...
export default function NuevaEspecialidadPage() {
  ...
  const handleSubmit = async (e: React.BaseSyntheticEvent) => {
    ...
    const servicios         = form.servicios.split('\n').map(s => sanitizeText(s, 100)).filter(Boolean)
    ...
    const { error: dbErr } = await supabase
      .from('especialidades')
      .insert([{ nombre, slug, descripcion_corta, descripcion, imagen_url, icono, servicios, activo: form.activo, orden: Number(form.orden) }])

    if (dbErr) {
      console.error('[Especialidades] db.insert:', dbErr)
      setError(dbErr.code === '23505' ? 'Ya existe una especialidad con ese slug.' : 'No se pudo guardar. Inténtalo de nuevo.')
      setSaving(false)
      return
    }
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-white">Nueva especialidad</h1>
          <p className="text-gray-400 text-sm">Agrega una especialidad al directorio</p>
        </div>
      </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 flex flex-col gap-4">
          ...
          <div>
            ...
            <textarea name="descripcion_corta" value={form.descripcion_corta} onChange={onChange}
              rows={2} maxLength={300} disabled={saving} className={`${inputCls} resize-none`} />
          </div>

          <div>
            <label className={labelCls}>Descripción larga</label>
            <textarea name="descripcion" value={form.descripcion} onChange={onChange}
              rows={4} maxLength={2000} disabled={saving} className={`${inputCls} resize-none`} />
          </div>
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={saving}
            ...
            {saving
              ...
              : <><span className="material-symbols-outlined text-[18px]">save</span>Guardar especialidad</>
            }
          </button>
          <Link href="/admin/especialidades"
