...
export default function UnidadesAdminPage() {
  ...
  const fetchItems = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .order('orden')
    if (error) { console.error('[Unidades] db.select:', error); setError('No se pudieron cargar las unidades.') }
    else setItems(data ?? [])
    setLoading(false)
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white">Unidades Médicas</h1>
          <p className="text-gray-400 mt-1 text-sm">{items.length} unidad{items.length !== 1 ? 'es' : ''} registrada{items.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      {items.length === 0 && !error ? (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-12 text-center">
          <div className="w-16 h-16 rounded-2xl bg-teal-500/10 border border-teal-500/20 flex items-center justify-center mx-auto mb-4">
            <span className="material-symbols-outlined text-teal-400 text-[32px]">business</span>
          </div>
          <p className="text-gray-400 mb-4">No hay unidades registradas</p>
          <Link href="/admin/unidades/nuevo"
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-800">
                <th className="text-left px-6 py-3 text-gray-500 text-xs font-semibold uppercase tracking-wider">Unidad</th>
            ...
            <tbody className="divide-y divide-gray-800">
              {items.map(item => (
                <tr key={item.id} className="hover:bg-gray-800/40 transition">
                    ...
                    <div className="flex items-center gap-3">
                      {item.imagen_url ? (
                        ...
                        <div className="w-10 h-10 rounded-lg bg-teal-500/20 border border-teal-500/30 flex items-center justify-center text-teal-400 shrink-0">
                          <span className="material-symbols-outlined text-[18px]">{item.icono || 'local_hospital'}</span>
                        </div>
                      )}
                      <div>
                        <p className="text-white text-sm font-medium">{item.nombre}</p>
                        {item.descripcion && (
                          <p className="text-gray-500 text-xs truncate max-w-xs">{item.descripcion}</p>
                        )}
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 hidden md:table-cell">
                    <span className="text-gray-500 text-xs font-mono">/unidades/{item.slug}</span>
                  </td>
                  <td className="px-6 py-4">
                    <button
                  ...
                  <td className="px-6 py-4">
                    <Link href={`/admin/unidades/${item.id}`}
