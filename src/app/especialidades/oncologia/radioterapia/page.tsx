...
export default function RadioterapiaPrecisionPage() {
  return (
    <main>
        ...
        <nav aria-label="Breadcrumb">
          <ol className="flex items-center gap-xs type-label text-on-surface-variant flex-wrap">
            ...
            <li className="flex items-center gap-xs" aria-current="page">
              <span className="material-symbols-outlined text-sm">chevron_right</span>
              <span className="text-tertiary font-medium">Radioterapia de Precisión</span>
            </li>
          </ol>
        </nav>
      </div>
      ...
      <section className="relative w-full h-150 md:h-175 flex items-center justify-center overflow-hidden bg-slate-900">
        <div className="absolute inset-0 z-0">
          <img
            alt="Sala de radioterapia con acelerador lineal de precisión"
        ...
        <div className="relative z-10 max-w-7xl w-full mx-auto px-margin-mobile md:px-margin-desktop">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-sm px-md py-sm rounded-full glass-panel text-tertiary-fixed type-label mb-md border border-tertiary-fixed/30">
              ...
              Centro Oncológico · Radioterapia
            </div>
            ...
            <p className="type-body-lg text-slate-300 mb-lg">
              Tratamientos submilimétricos con el acelerador lineal TrueBeam que atacan el tumor con exactitud máxima, preservando el tejido sano circundante y reduciendo los efectos secundarios.
            </p>
        ...
        <div className="container-page">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-xl items-center">
            ...
            <div className="flex flex-col gap-md">
              ...
              <h2 className="type-display text-on-surface">¿En qué consiste la radioterapia de precisión?</h2>
              <p className="type-body-lg text-on-surface-variant">
                La radioterapia de precisión utiliza haces de radiación de alta energía cuidadosamente dirigidos para destruir las células cancerígenas o inhibir su crecimiento, con una exactitud submilimétrica que protege al máximo el tejido sano adyacente.
              </p>
              <p className="type-body text-on-surface-variant">
                Nuestro sistema TrueBeam de Varian incorpora guía por imagen en tiempo real (IGRT), sincronización con la respiración del paciente y modulación de intensidad (IMRT/VMAT) para entregar la dosis óptima en el menor número de sesiones posible.
              </p>
              <div className="flex flex-col gap-sm mt-sm">
                {[
                  { icon: 'schedule',       text: 'Sesiones de 15 a 30 minutos — tratamiento ambulatorio' },
                  { icon: 'verified',       text: 'Sin cirugía ni anestesia general' },
                  { icon: 'local_hospital', text: 'Planificación dosimétrica 3D individualizada para cada caso' },
                ].map(({ icon, text }) => (
            ...
            <div className="relative rounded-3xl overflow-hidden shadow-2xl h-150 order-first lg:order-last">
              <img
                alt="Técnico de radioterapia operando el acelerador lineal"
                className="absolute inset-0 w-full h-full object-cover"
                src="/imagenes/oncologia.png"
              />
              ...
              <div className="absolute bottom-md left-md right-md">
                <div className="inline-flex items-center gap-xs bg-surface/20 backdrop-blur-md rounded-full px-sm py-xs text-white type-label">
                  ...
                  Sala de Radioterapia · Aura Medical
                </div>
              </div>
            </div>
        ...
        <div className="container-page">
          <div className="section-header">
            ...
            <p className="type-body text-on-surface-variant max-w-2xl mx-auto">
              La radioterapia de precisión puede indicarse en múltiples situaciones oncológicas, tanto como tratamiento único como en combinación con otros enfoques.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-md max-w-5xl mx-auto">
            {[
              { icon: 'health_metrics',    title: 'Tumor localizado',              desc: 'Cáncer en etapa inicial sin metástasis sistémica, donde la precisión del haz maximiza el control local.' },
              { icon: 'biotech',           title: 'Adyuvante postquirúrgico',      desc: 'Tras una cirugía oncológica para eliminar células residuales y reducir el riesgo de recurrencia.' },
              { icon: 'medication_liquid', title: 'Combinado con quimioterapia',   desc: 'La radiosensibilización con quimio potencia el efecto destructivo sobre el tumor.' },
              { icon: 'psychology',        title: 'Metástasis cerebrales',         desc: 'Radiocirugía estereotáctica (SRS) para lesiones cerebrales múltiples con mínima toxicidad.' },
              { icon: 'monitor_heart',     title: 'Tumores de columna',            desc: 'Radioterapia estereotáctica corporal (SBRT) para lesiones vertebrales en pocas sesiones.' },
              { icon: 'water_drop',        title: 'Control paliativo del dolor',   desc: 'Reducción de metástasis óseas dolorosas cuando la curación no es el objetivo primario.' },
            ].map(({ icon, title, desc }) => (
        ...
        <div className="container-page">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-xl items-center">
            ...
            <div className="relative rounded-3xl overflow-hidden shadow-2xl h-150">
              <img
                alt="Acelerador lineal TrueBeam de última generación"
                className="absolute inset-0 w-full h-full object-cover"
            ...
            <div className="flex flex-col gap-md">
              ...
              <h2 className="type-display text-on-surface">Por qué elegirnos para su radioterapia</h2>
              <p className="type-body-lg text-on-surface-variant">
                Nuestro Centro Oncológico reúne la tecnología más avanzada y un equipo multidisciplinario certificado para ofrecer el tratamiento más eficaz y seguro.
              </p>
              <div className="flex flex-col gap-md">
                {[
                  { icon: 'precision_manufacturing', color: 'text-tertiary',   bg: 'bg-tertiary/10',   title: 'Acelerador TrueBeam con IGRT y IMRT',      desc: 'Guía por imagen en tiempo real y modulación de intensidad para la dosis exacta en cada sesión.' },
                  { icon: 'group',                   color: 'text-secondary',  bg: 'bg-secondary/10',  title: 'Comité Multidisciplinario de Tumores',      desc: 'Oncólogos, radioterapeutas, cirujanos y psicólogos revisan cada caso en conjunto cada semana.' },
                  { icon: 'psychology',              color: 'text-primary',    bg: 'bg-primary/10',    title: 'Apoyo Integral al Paciente',               desc: 'Acompañamiento emocional, nutricional y de rehabilitación durante todo el tratamiento.' },
                  { icon: 'verified',                color: 'text-error',      bg: 'bg-error/10',      title: 'Certificación Internacional JCI',          desc: 'Protocolos de seguridad validados por la Joint Commission International en radioprotección.' },
      ...
      <section className="section bg-slate-900 text-white overflow-hidden relative">
        ...
        <div className="container-page relative z-10 text-center flex flex-col items-center gap-md">
          <div className="w-16 h-16 rounded-full bg-tertiary/20 border border-tertiary-fixed/30 flex items-center justify-center">
            <span className="material-symbols-outlined text-tertiary-fixed text-[32px]" style={{ fontVariationSettings: "'FILL' 1" }}>radiology</span>
          </div>
          <h2 className="type-display font-bold">¿Necesita una evaluación oncológica?</h2>
          <p className="type-body-lg text-slate-300 max-w-2xl">
            Nuestro equipo de radioterapeutas y oncólogos está disponible para analizar su caso, explicarle el proceso y diseñar el plan de tratamiento más adecuado.
          </p>
