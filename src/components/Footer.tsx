import { WA_NUMBER, CLINIC_PHONE, CLINIC_EMAIL, CLINIC_ADDRESS } from '@/lib/constants'
...
export default function Footer({ contacto = {}, redes = {}, footer = {} }: FooterProps) {
  const telefono  = contacto.telefono  || CLINIC_PHONE
  const email     = contacto.email     || CLINIC_EMAIL
  const direccion = contacto.direccion || CLINIC_ADDRESS
  const whatsapp  = contacto.whatsapp  || WA_NUMBER
  const urgencias = contacto.urgencias || '911'
  const copyright = footer.copyright   || '© 2026 Aura Medical Private Clinic. Todos los derechos reservados.'

  const fbUrl  = redes.facebook  || 'https://www.facebook.com/AuraMedicalMX'
  const igUrl  = redes.instagram || 'https://www.instagram.com/auramedical.mx'
  const ytUrl  = redes.youtube   || 'https://www.youtube.com/@AuraMedicalMX'
  const liUrl  = redes.linkedin  || 'https://www.linkedin.com/company/aura-medical-mx'

  return (
    <footer id="footer" className="w-full bg-on-background border-t border-outline/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-xl mb-xl">
          ...
          <div className="md:col-span-4 flex flex-col gap-md">
            ...
            <p className="type-body text-surface-variant/70">
              {footer.descripcion || 'Redefiniendo los estándares de la medicina privada a través de la excelencia clínica, tecnología avanzada y un trato verdaderamente humano.'}
            </p>

            {/* WhatsApp CTA */}
            <a
              href={`https://wa.me/${whatsapp}?text=Hola%2C%20me%20gustar%C3%ADa%20obtener%20m%C3%A1s%20informaci%C3%B3n%20sobre%20Aura%20Medical.`}
              target="_blank"
            ...
            <div className="flex items-center gap-md mt-xs">
              ...
              {/* Facebook */}
              <a href={fbUrl} target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="text-surface-variant/50 hover:text-primary-fixed transition-colors">
              ...
              {/* Instagram */}
              <a href={igUrl} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="text-surface-variant/50 hover:text-primary-fixed transition-colors">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              {/* YouTube */}
              <a href={ytUrl} target="_blank" rel="noopener noreferrer" aria-label="YouTube" className="text-surface-variant/50 hover:text-primary-fixed transition-colors">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              {/* LinkedIn */}
              <a href={liUrl} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" className="text-surface-variant/50 hover:text-primary-fixed transition-colors">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          <div className="md:col-span-3 flex flex-col gap-sm">
            <h4 className="footer-heading">{footer.columna_1_titulo || 'Especialidades'}</h4>
          ...
          <div className="md:col-span-5 flex flex-col gap-sm">
            <h4 className="footer-heading">{footer.columna_2_titulo || 'Urgencias y Contacto'}</h4>
            ...
            <div className="bg-surface-variant/10 p-md rounded-xl border border-surface-variant/20 mb-sm">
              ...
              <a href={`tel:${urgencias}`} className="type-headline text-surface-bright hover:text-primary-fixed transition-colors">
                {urgencias}
              </a>
            </div>
            ...
            <a
              href={`https://wa.me/${whatsapp}?text=Hola%2C%20me%20gustar%C3%ADa%20obtener%20informaci%C3%B3n.`}
              target="_blank"
              rel="noopener noreferrer"
