...
export default function EditarUnidadPage() {
  ...
  const [form, setForm] = useState({
    nombre: '', descripcion: '', imagen_url: '', icono: '', activo: true, orden: 0,
  })
  ...
  useEffect(() => {
    const fetch = async () => {
      const { data, error: dbErr } = await supabase
        ...
        .single()
      if (dbErr || !data) {
        console.error('[Unidades] db.select:', dbErr)
  ...
  const handleSubmit = async (e: React.BaseSyntheticEvent) => {
    ...
    const { error: dbErr } = await supabase
      .from('unidades')
      .update({ nombre, descripcion, imagen_url, icono, activo: form.activo, orden: Number(form.orden) })
      .eq('id', id)

    if (dbErr) {
      console.error('[Unidades] db.update:', dbErr)
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-white">Editar unidad</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 flex flex-col gap-4">
          ...
          <div>
            ...
            <input value={`/unidades/${slug}`} readOnly
          ...
          <div>
            ...
            <textarea name="descripcion" value={form.descripcion} onChange={onChange}
              rows={3} maxLength={1000} disabled={saving}
